// ABOUTME: SSR page component for serving context stack conversions with SEO optimization
...
export default async function ContextConversion({ params, searchParams }: ContextConversionProps) {
        ...
        <article className="bg-white rounded-lg shadow-sm p-8">
          <header className="mb-8 pb-6 border-b border-gray-200">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <a href={`/context/${conversion.slug}.md`}
              ...
              <a href={`/context/${conversion.slug}.xml`}
                 className="inline-flex items-center px-3 py-1 bg-orange-100 text-orange-800 rounded-full text-xs hover:bg-orange-200 transition-colors">
                🔖 View as XML
              </a>
            </div>
          </header>
...
export async function generateMetadata({ params }: ContextConversionProps) {
  const { slug } = await params;
  const conversion = await getConversion(slug);
  
  if (!conversion) {
    return {
      title: 'Context Stack Not Found | ctxt.help',
    };
  }
  ...
  return {
    title: `${conversion.title} | Context Stack | ctxt.help`,
    description: metaDescription,
    openGraph: {
      title: conversion.title,
      description: metaDescription,
      url: `https://ctxt.help/context/${conversion.slug}`,
      type: 'article',
    ...
    canonical: `https://ctxt.help/context/${conversion.slug}`,
  };
}