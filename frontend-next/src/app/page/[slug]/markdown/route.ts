// ABOUTME: Route handler for serving markdown versions of single page conversions
...
*Return to homepage: https://ctxt.help*`;
  ...
  if (isContextStack(conversion)) {
    ...
    return NextResponse.redirect(new URL(`/context/${slug}.md`, request.url), 301);
  }
...
function formatMarkdownContent(conversion: Conversion, isExplicitRequest: boolean = false): string {
  const viewAsHtmlLink = isExplicitRequest ? 
    `*View as HTML: https://ctxt.help/page/${conversion.slug}*\n` : '';
...
*Markdown link: https://ctxt.help/page/${conversion.slug}.md*
${viewAsHtmlLink}`;
}